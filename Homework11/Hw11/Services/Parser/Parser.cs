...
using static Hw11.Services.Parser.Checker;
    ...
    public class Parser
    {
        ...
        static public Queue<string> Parse(string expression)
        {
            ...
            foreach (var elem in splittedExpression)
            {
                ...
                {
                    if (elem.Length == 1)
                        throw new InvalidSymbolException(MathErrorMessager.UnknownCharacterMessage(elem[0]));
                    else
                        throw new InvalidNumberException(MathErrorMessager.NotNumberMessage(elem));
                }
            }
